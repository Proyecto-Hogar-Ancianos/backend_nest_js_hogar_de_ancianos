...
import { AuditReport, AuditReportType } from '../../domain/audit';
...
export class AuditReportsService {
  ...
  async getAuditReports(filterDto?: AuditReportFilterDto): Promise<AuditReportResponse[]> {
    ...
    if (filterDto?.startDate && filterDto?.endDate) {
      ...
      whereConditions.createAt = { $gte: new Date(filterDto.startDate) };
    } else if (filterDto?.endDate) {
      whereConditions.createAt = { $lte: new Date(filterDto.endDate) };
    }

    if (filterDto?.generatorId) {
      whereConditions.idGenerator = filterDto.generatorId;
    }
